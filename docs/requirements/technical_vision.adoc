...
|Check-Listen für Angestellte *oder Admin?* automatisch anlegen/zuordnen| Hoch |für Admin/Angestellte des Stura werden einzelne Aufgaben automatisch nach Mitgliedsaufnahme angelegt, welche abgehakt werden können. Diese Aufgaben sind: die Einrichtung der Stura Karte für den Zugriff auf das A-Gebäude (Kartenleser); Ausgeben *Zuordnen?* der Schlüssel für die passenden Räume; E-Mail  (Datengeheimnis, Mitwirkungsordnung unterschreiben) an neue Mitglieder *automatisch?* versenden; Hochladen der ausgefüllten Unterlagen | xx
|Weitergabe des Telefonnummer der Mitgliedern erfragen | Hoch | Erfragung ob die Telefonnummer von neue Mitgliedern im Bedarfsfall weitergegeben werden kann; eintragen ob Telefonnummer weitergegeben werden darf| xx
