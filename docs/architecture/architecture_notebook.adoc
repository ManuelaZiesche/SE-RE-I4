...
Die persönlichen Daten der eingespeicherten Kandidaten und Mitgliedern müssen entsprechend DSGVO behandelt werden und es wird sichergestellt, dass diese nur von Nutzern mit entsprechenden Berechtigungen eingesehen werden können. +

Da das endgültige System vom Stura der HTW Dresden selbst genutzt werden soll, ist es ausreichend, wenn es zu deren Arbeitszeiten läuft. Es muss also nicht zu jeder zeit verfügbar sein.
...
.neu: +
. Wir nutzen Python als Programmiersprache, da die bestehenden Teile der Anwendung in dieser Sprache programmiert wurden und wir einen Mehraufwand im Sinne einer Umstrukturierung vermeiden wollten. Außerdem ist sie objektorientierte und besitzt eine verhältnismäßig leichte Syntax.
...
. SQLite wird als Programmierbibliothek aufgegriffen, da es standartmäßig von Django unterstützt wird und von der Syntax sehr dem SQL-Standard ähnelt, mit dem einige Gruppenmitglieder schon intensiveren Kontakt hatten. Abgesehen davon ist SQLite sowohl mit verschiedenen Betriebssystemen von Smartphones, sowie mit unterschiedlichsten Browsern kompatibel und unterstützt so das Erreichen von mehr Nutzern, die sich durch ihre verschiedenen Voraussetzungen auszeichnen.
